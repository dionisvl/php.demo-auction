...
deploy:
	ssh ${HOST} -p ${PORT} -i "${KEY}" 'rm -rf site_${BUILD_NUMBER}'
	ssh ${HOST} -p ${PORT} -i "${KEY}" 'mkdir site_${BUILD_NUMBER}'
	scp -P ${PORT} -i "${KEY}" docker-compose-production.yml ${HOST}:site_${BUILD_NUMBER}/docker-compose.yml
	ssh ${HOST} -p ${PORT} -i "${KEY}" 'cd site_${BUILD_NUMBER} && echo "COMPOSE_PROJECT_NAME=auction" >> .env'
	...
	ssh ${HOST} -p ${PORT} -i "${KEY}" 'cd site_${BUILD_NUMBER} && echo "API_MAILER_PASSWORD=${API_MAILER_PASSWORD}" >> .env'
	ssh ${HOST} -p ${PORT} -i "${KEY}" 'cd site_${BUILD_NUMBER} && echo "API_MAILER_FROM_EMAIL=${API_MAILER_FROM_EMAIL}" >> .env'
	ssh ${HOST} -p ${PORT} -i "${KEY}" 'cd site_${BUILD_NUMBER} && echo "SENTRY_DSN=${SENTRY_DSN}" >> .env'
